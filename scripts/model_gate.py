...
def main() -> None:
    ...
    #
    # КОГДА ВЗВОДИТЬ БЛОКИРОВКУ. Не раньше, чем выполнены оба условия:
    # (1) метрика способна погаснуть — вердикт #158 считается по
    #     взвешенному TVD дня на окне CONFOUNDING_WINDOW_DAYS, а не по
    #     максимуму за всю историю (тот держали намертво июльские когорты,
    #     которые из выборки уже не уйдут);
    # (2) она фактически зелёная 4 ретрейна подряд — то есть равномерный сбор
    #     это устойчивое состояние, а не удачная неделя.
    # Взвести блокировку раньше = заморозить релизы модели на месяцы и
    # получить гейт, который снимут не разбираясь.
