"""Чек сервера (GitHub Actions space-keepalive.yml) → алерт в Telegram.

Логика: раз в 6 часов, вместе с keepalive-пингом, опрашиваем /api/health и
проверяем не только код ответа, но и содержимое (модель загружена, webhook
жив). Сообщение шлём только при смене состояния (упал/ожил/деградировал),
чтобы не спамить; текущее состояние живёт между запусками в actions/cache
(файл --state-file).

ИСТОРИЯ: воркфлоу health-check.yml, на который ссылался этот докстринг, в
репозитории никогда не существовал — скрипт был написан, покрыт тестами и
ни разу не запущен. Теперь он вызывается из space-keepalive.yml, который до
этого умел только «ответил ли сервер 200».
