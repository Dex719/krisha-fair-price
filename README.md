...
![tests](https://img.shields.io/badge/tests-223_passed-2ea44f)
...
Метрики модели v0.2.0 на **честном сплите**: перевыставленные дубли схлопнуты, train/test разбит по зданиям (`GroupShuffleSplit`) — модель не видит квартиры из тестового дома при обучении. Живая медианная ошибка текущей модели публикуется в `/api/health`.
...
tests/              # 223 теста: парсеры (фикстуры с реальной разметкой), фичи, БД, бот, API, фронт-приёмка, train-smoke
```
