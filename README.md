...
| `GET` | `/api/metrics` | что происходит прямо сейчас: rps, коды ответов, p50/p95/p99 по маршрутам, попадания в кэш предикта |
...
**Под нагрузкой.** Расчётный сценарий — ссылку постят в большой телеграм-канал:
тысяча человек за две минуты, почти все открывают одно и то же объявление.

- *Горячие ответы* живут в памяти процесса: `/api/health` — 60 с, `/api/stats` и
  `/api/heatmap` — 10 минут, готовый разбор объявления — 10 минут по id лота.
  Кэш с single-flight: в момент истечения TTL пересчёт делает один поток,
  остальные ждут его результат, а не идут в базу толпой; если пересчёт упал —
  отдаётся последнее удачное значение вместо 503.
- *Статика отдаётся из памяти предсжатой* (`api/static_cache.py`): gzip считается
  один раз при старте, а не на каждый запрос. Это самая дорогая по CPU часть
  главной страницы — на локальном замере сжатие на лету стоило половины
  пропускной способности `/`. ETag считается от содержимого, у сжатого и
  несжатого вариантов он разный; `If-None-Match` → 304.
- *Тяжёлый предикт ходит через одну калитку* (`predict_gate.py`) — и веб, и бот:
  кэш по id лота, single-flight, негативный кэш на битые ссылки, лимит
  одновременных скрейпов и короткий сетевой бюджет (5 с вместо краулерных 30).
  Ответ из кэша отдаётся раньше проверки rate-limit, а если все слоты заняты
  дольше `PREDICT_WAIT_S` — честные 503 + `Retry-After` вместо висящего спиннера.
- *Лимиты раздельные*: `/api/demo` (его дёргает каждая загрузка главной) не делит
  бюджет с `/api/predict` — за CGNAT мобильного оператора под одним IP сидит
  целый город.
- Приложение поднимается в `WEB_CONCURRENCY` процессов (по умолчанию 2).
  Замер — `python scripts/loadtest.py <url>`, что происходит на живом
  проде — `/api/metrics`.
...
tests/              # 590 тестов: парсеры (фикстуры с реальной разметкой), фичи, БД, бот, API, фронт-приёмка, train-smoke, артефакты модели
tests/e2e/          # 65 e2e поверх настоящего uvicorn: браузерные (pytest-playwright) + контракт API/статики через живой сокет, из них 3 live-smoke
```
