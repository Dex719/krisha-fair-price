...
TEST_WINDOW_DAYS = 14
# Окно, на котором проверяется спутанность времени с составом данных
# (validity.time_confounding). Шире тестового: перекос виден только в
# сравнении дней между собой, а на 14 днях сравнивать почти нечего. Ýже всей
# истории: вопрос «валидна ли ТЕКУЩАЯ оценка», а не «был ли когда-нибудь
# кривой сбор» — на второй ответ «да» останется навсегда.
CONFOUNDING_WINDOW_DAYS = 30
...
def train(
    ...
    # Спутанность считаем на окне, которое реально участвует в оценке: тест —
    # последние TEST_WINDOW_DAYS дней, а сравнивается он с ближайшей к нему
    # частью обучения. Проверка «по всей истории» отвечает на другой вопрос
    # («был ли когда-нибудь перекошенный сбор») и на проде отвечает «да»
    # навсегда: июльские когорты из выборки уже никуда не денутся, а гейт,
    # который невозможно погасить, — это гейт, который снимут не разобравшись.
    # Обе цифры уезжают в мету: вердикт по окну, история — рядом, диагностикой.
    confounding = time_confounding(df, window_days=CONFOUNDING_WINDOW_DAYS)
    confounding_history = time_confounding(df)
    ...
    if not temporal_valid:
        logger.warning(
            ...
            "спутанность времени с районом на окне %s дней: %s (взвешенный TVD дня %s). "
            "Число нельзя читать как «среднюю ошибку модели по городу» и нельзя "
            "гейтить им релизы",
            test_repr["worst_tvd"], test_repr["threshold"], CONFOUNDING_WINDOW_DAYS,
            confounding["confounded"], confounding.get("weighted_day_tvd"),
        )
    ...
    metrics = {
        ...
        # Спутана ли временная ось с составом данных на окне оценки. Пока
        # confounded=true, rolling-origin померяет перенос между районами, а
        # не обобщение во времени. Проверка гаснет сама, когда сбор становится
        # равномерным (issue #152), — дату для этого знать не нужно.
        "time_confounding": confounding,
        # То же по ВСЕЙ истории. Отдельным полем, потому что отвечает на другой
        # вопрос: пригодны ли для дневного rolling-origin backtest'а старые
        # участки выборки. Июльские когорты (сбор по алфавиту с лимитом
        # 1000/день) держат этот показатель красным независимо от того, как
        # собираются данные сегодня.
        "time_confounding_history": confounding_history,
