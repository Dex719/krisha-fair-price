...
def sweep(
    ...
    stats = {
        ...
        # ЭФФЕКТИВНЫЙ (после подрезки реальной очередью и бюджетом
        # fit_detail_caps). Публиковать эффективный под именем заявленного
        # нельзя: при очереди мельче потолка эффективный сжимается до
        # очереди, и сравнение «докачано == потолок» становится тождеством —
        # детектор «упёрлись в лимит» (#154) краснел бы навсегда ровно в
        # момент успеха разгребания. Детектор сравнивает с эффективным И
        # требует непустой очереди после прохода (daily_report._hit_cap).
        "max_new_details": want_new,
        "max_new_effective": eff_max_new,
    ...
    record_sweep_run(
        {
            ...
            **{k: stats.get(k) for k in (
                ...
                "max_new_details", "max_new_effective", "detail_queue_after",
                "price_changes", "delisted", "recovery_pass", "suspicious",
            )},
        },
        db_path,
    )
