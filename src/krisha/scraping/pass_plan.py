...
# < 10 000 запросов в сутки и < 0.5 rps в среднем. Проход один в сутки на
# тип сделки, поэтому суточный потолок = потолок на проход. Оба ограничения
...
def fit_detail_caps(
    ...
    *,
    ...
    by_time = max(
        0,
        int((budget_s - elapsed_s - _reserve(budget_s, reserve_s)) // max(t_detail, 0.1)),
    )
