...
def _weekly_trend(
    ...
    for i in range(max_weeks - 1, -1, -1):
        ...
        # Раньше для i == 0 конец окна растягивался до «сейчас», а week_t
        # оставался целым номером недели: последняя точка покрывала 7–14 дней,
        # но на оси времени стояла ровно на отметке одной недели. Наклон
        # тренда (и экстраполяция в forecast.py) считался по смещённой точке.
        # Держим все окна ровно недельными — свежесть страдает максимум на
        # 7 дней, зато t честно соответствует данным.
