...
def test_predict_api_returns_real_price_history(monkeypatch):
    ...
    monkeypatch.setattr(predict_gate, "predict_from_url", lambda url, live_vision=False, timeout=None, on_attempt=None: _predict_payload(history))
...
def test_predict_api_keeps_empty_price_history_without_synthetic_rows(monkeypatch):
    monkeypatch.setattr(predict_gate, "predict_from_url", lambda url, live_vision=False, timeout=None, on_attempt=None: _predict_payload([]))
    app_module._rate.clear()
